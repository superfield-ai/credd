...
## 9. Constraints
- **Zero Required External Dependencies**: The system runs as a single binary without requiring external SaaS services, orchestration, or database clusters. **Dev mode** operates with zero external dependencies (local file audit sink, OS keychain/software KEK). **Production hardening** adds optional operator-provided infrastructure you control: WORM object store (S3/GCS Object Lock) in a separate account for the audit chain, and KMS/TPM/Secure Enclave for KEK sealing. This distinguishes "no required SaaS/DB cluster" from "no platform/operator-provided storage/KMS."
